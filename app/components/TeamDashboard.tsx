...
const voetbalNlStandingsUrl = "https://www.voetbal.nl/team/T1719192193/stand";
const voetbalNlAppTeamUrl = "https://apps.sportlink.com/voetbalnl/team_details/T1719192193";

function openVoetbalNlStandings(event: MouseEvent<HTMLAnchorElement>) {
  const isMobile = /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)
    || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);
  if (!isMobile) return;

  event.preventDefault();
  window.location.assign(voetbalNlAppTeamUrl);
}
...
const teamLogos: Record<string, string> = {
  "sv twello 2": "./sv-twello-mark-transparent.png",
  "columbia 3": "./team-logos/columbia.png",
  "davo 2": "./team-logos/davo.png",
  "tka 2": "./team-logos/tka.png",
  "tka 3": "./team-logos/tka.png",
  "voorwaarts t 5": "./team-logos/voorwaarts.png",
  "sportclub deventer 3": "./team-logos/sportclub-deventer.png",
  "epse 2": "./team-logos/epse.png",
  "heeten 5": "./team-logos/heeten.png",
  "sc klarenbeek 3": "./team-logos/sc-klarenbeek.png",
  "loenermark 3": "./team-logos/loenermark.png",
  "sallandia 2": "./team-logos/sallandia.png",
...
const awardEmblems: Record<string, string> = { Trainingsbeest: "training", Scherpschutter: "goals", Assistkoning: "assists", Trainingsspook: "ghost", "Onzichtbare man": "invisible", Uitslaper: "late" };
function AwardEmblem({ awardTitle }: { awardTitle?: string }) { const emblem = awardTitle ? awardEmblems[awardTitle] : undefined; return emblem ? <span className={`award-emblem award-emblem-${emblem}`} aria-hidden="true"/> : null; }
function Leader({ title, awardTitle, players, score, displayName = (player) => player.name }: { title: string; awardTitle?: string; players: Player[]; score: number; displayName?: (player: Player) => string }) { return <article className="leader-card"><div className="award-card-heading"><AwardEmblem awardTitle={awardTitle}/><div><span className="leader-kind">Meeste {title.toLowerCase()}</span>{awardTitle && <strong className="leader-award">{awardTitle}</strong>}</div></div><span className="leader-score">{score}</span><div className="leader-name">{players.length ? players.map(displayName).join(" · ") : "Nog geen invoer"}</div></article>; }
function Loser({ label, awardTitle, players, score, displayName = (player) => player.name, maxNames = 3, showScore = true, inlineNames = false, inlineScores = false, cardScore }: { label: string; awardTitle: string; players: Player[]; score: (player: Player) => number; displayName?: (player: Player) => string; maxNames?: number; showScore?: boolean; inlineNames?: boolean; inlineScores?: boolean; cardScore?: number | string }) { return <article className="loser-card"><div className="award-card-heading"><AwardEmblem awardTitle={awardTitle}/><div><span className="loser-title">{label}</span><strong className="loser-award">{awardTitle}</strong></div></div>{cardScore !== undefined && <span className="leader-score">{cardScore}</span>}{players.length > maxNames ? <p>{players.length} spelers delen de eerste plaats.</p> : players.length && inlineNames ? <div className="leader-name">{players.map((player) => `${displayName(player)}${inlineScores ? ` (${score(player)})` : ""}`).join(" · ")}</div> : players.length ? <ol>{players.map((player) => <li key={player.id}><strong>{displayName(player)}</strong>{showScore && <b>{score(player)}</b>}</li>)}</ol> : <p>Nog geen invoer</p>}</article>; }
...
function TeamLogo({ team }: { team: string }) { const normalized = team.trim().toLocaleLowerCase("nl"); const source = teamLogos[normalized]; if (!source) return null; const isTwello = normalized === "sv twello 2"; const isWide = ["voorwaarts t 5", "epse 2"].includes(normalized); return <span className={`fixture-team-logo-wrap${isTwello ? " twello" : ""}${isWide ? " wide" : ""}`} aria-hidden="true"><img className="fixture-team-logo" src={source} alt=""/>{isTwello && <small>SV TWELLO</small>}</span>; }
function Fixture({ match, showResult = true, emphasis = false, contributors, manOfTheMatchLabel, showMvp = true, onOpen }: { match: Match; showResult?: boolean; emphasis?: boolean; contributors?: MatchContributors; manOfTheMatchLabel?: string; showMvp?: boolean; onOpen?: () => void }) { const finalResult = showResult && hasFinalResult(match.result); const contributorLabel = (entries: MatchContributors["goals"]) => entries.map((entry) => `${entry.name}${entry.count > 1 ? ` ×${entry.count}` : ""}`).join(" · "); const content = <><div className="fixture-meta"><span>{match.competition || "Wedstrijd"}</span><span>{formatDate(match.date)} {match.time && `· ${match.time}`}</span></div><div className={finalResult ? "fixture-teams" : "fixture-teams upcoming"}><span className="fixture-team fixture-team-home"><TeamLogo team={match.home}/><strong>{match.home || "Thuisteam onbekend"}</strong></span>{finalResult ? <span className="fixture-score">{match.result.trim()}</span> : <span className="fixture-separator" aria-label="Nog niet gespeeld">vs</span>}<span className="fixture-team fixture-team-away"><strong>{match.away || "Uitteam onbekend"}</strong><TeamLogo team={match.away}/></span></div>{finalResult && contributors && (contributors.goals.length > 0 || contributors.assists.length > 0) && <div className="match-contributors">{contributors.goals.length > 0 && <div><span aria-hidden="true">⚽</span><span>{contributorLabel(contributors.goals)}</span></div>}{contributors.assists.length > 0 && <div><span aria-hidden="true">🎯</span><span>{contributorLabel(contributors.assists)}</span></div>}</div>}{showMvp && finalResult && match.manOfTheMatch?.trim() && <div className="match-mvp"><span aria-hidden="true">⭐</span> {(manOfTheMatchLabel || match.manOfTheMatch).trim()}</div>}</>; const className = `fixture-card${emphasis ? " latest-result" : ""}${onOpen ? " interactive" : ""}`; return onOpen ? <button type="button" className={className} onClick={onOpen} aria-label={`Bekijk wedstrijdstatistieken van ${match.home} tegen ${match.away}`}>{content}<span className="fixture-open">Bekijk wedstrijdstatistieken <b aria-hidden="true">›</b></span></button> : <article className={className}>{content}</article>; }
