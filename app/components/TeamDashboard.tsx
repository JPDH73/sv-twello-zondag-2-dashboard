...
function Loser({ label, awardTitle, players, score, displayName = (player) => player.name, maxNames = 3, showScore = true, inlineNames = false, cardScore }: { label: string; awardTitle: string; players: Player[]; score: (player: Player) => number; displayName?: (player: Player) => string; maxNames?: number; showScore?: boolean; inlineNames?: boolean; cardScore?: number | string }) { return <article className="loser-card"><div className="award-card-heading"><AwardEmblem awardTitle={awardTitle}/><div><span className="loser-title">{label}</span><strong className="loser-award">{awardTitle}</strong></div></div>{cardScore !== undefined && <span className="leader-score">{cardScore}</span>}{players.length > maxNames ? <p>{players.length} spelers delen de eerste plaats.</p> : players.length && inlineNames ? <div className="leader-name">{players.map(displayName).join(" · ")}</div> : players.length ? <ol>{players.map((player) => <li key={player.id}><strong>{displayName(player)}</strong>{showScore && <b>{score(player)}</b>}</li>)}</ol> : <p>Nog geen invoer</p>}</article>; }
