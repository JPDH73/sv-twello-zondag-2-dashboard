...
function Fixture({ match, showResult = true }: { match: Match; showResult?: boolean }) { return <article className="fixture-card"><div className="fixture-meta"><span>{match.competition || "Wedstrijd"}</span><span>{formatDate(match.date)} {match.time && `· ${match.time}`}</span></div><div className={showResult ? "fixture-teams" : "fixture-teams upcoming"}><strong>{match.home}</strong>{showResult && <span>{match.result || "–"}</span>}<strong>{match.away}</strong></div></article>; }
function ClubBadge() { return <span className="club-badge" aria-label="SV Twello"><span className="club-badge-mark"><img src="./sv-twello-mark.png" alt="" aria-hidden="true"/></span><small>SV Twello</small></span>; }
