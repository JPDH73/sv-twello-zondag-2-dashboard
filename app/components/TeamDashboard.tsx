...
function Loser({ label, awardTitle, players, score, displayName = (player) => player.name, maxNames = 3, showScore = true, inlineNames = false, cardScore }: { label: string; awardTitle: string; players: Player[]; score: (player: Player) => number; displayName?: (player: Player) => string; maxNames?: number; showScore?: boolean; inlineNames?: boolean; cardScore?: number | string }) { return <article className="loser-card"><span className="loser-title">{label}</span><strong className="loser-award">{awardTitle}</strong>{cardScore !== undefined && <span className="leader-score">{cardScore}</span>}{players.length > maxNames ? <p>{players.length} spelers delen de eerste plaats.</p> : players.length && inlineNames ? <div className="leader-name">{players.map(displayName).join(" · ")}</div> : players.length ? <ol>{players.map((player) => <li key={player.id}><strong>{displayName(player)}</strong>{showScore && <b>{score(player)}</b>}</li>)}</ol> : <p>Nog geen invoer</p>}</article>; }
...
function Fixture({ match, showResult = true, emphasis = false, contributors, manOfTheMatchLabel, showMvp = true, onOpen }: { match: Match; showResult?: boolean; emphasis?: boolean; contributors?: MatchContributors; manOfTheMatchLabel?: string; showMvp?: boolean; onOpen?: () => void }) { const finalResult = showResult && hasFinalResult(match.result); const contributorLabel = (entries: MatchContributors["goals"]) => entries.map((entry) => `${entry.name}${entry.count > 1 ? ` ×${entry.count}` : ""}`).join(" · "); const content = <><div className="fixture-meta"><span>{match.competition || "Wedstrijd"}</span><span>{formatDate(match.date)} {match.time && `· ${match.time}`}</span></div><div className={finalResult ? "fixture-teams" : "fixture-teams upcoming"}><span className="fixture-team fixture-team-home"><strong>{match.home || "Thuisteam onbekend"}</strong><TeamLogo team={match.home}/></span>{finalResult ? <span className="fixture-score">{match.result.trim()}</span> : <span className="fixture-separator" aria-label="Nog niet gespeeld">vs</span>}<span className="fixture-team fixture-team-away"><TeamLogo team={match.away}/><strong>{match.away || "Uitteam onbekend"}</strong></span></div>{finalResult && contributors && (contributors.goals.length > 0 || contributors.assists.length > 0) && <div className="match-contributors">{contributors.goals.length > 0 && <div><span aria-hidden="true">⚽</span><span>{contributorLabel(contributors.goals)}</span></div>}{contributors.assists.length > 0 && <div><span aria-hidden="true">🎯</span><span>{contributorLabel(contributors.assists)}</span></div>}</div>}{showMvp && finalResult && match.manOfTheMatch?.trim() && <div className="match-mvp"><span aria-hidden="true">⭐</span> {(manOfTheMatchLabel || match.manOfTheMatch).trim()}</div>}</>; const className = `fixture-card${emphasis ? " latest-result" : ""}${onOpen ? " interactive" : ""}`; return onOpen ? <button type="button" className={className} onClick={onOpen} aria-label={`Bekijk wedstrijdstatistieken van ${match.home} tegen ${match.away}`}>{content}<span className="fixture-open">Bekijk wedstrijdstatistieken <b aria-hidden="true">›</b></span></button> : <article className={className}>{content}</article>; }
